...
- ### 1. Prepare data
  ...
  You can also use your own runoff data to compute the climatology. The built-in dataset classes (`DailyBinDataset`, `NetCDFDataset`, `ERA5LandDataset`, etc.) all provide an `export_runoff_climatology()` method that aggregates any time-series runoff into a catchment-level mean-annual climatology NetCDF, which can then be fed directly into `estimate_river_geometry()`.
