...
- ### 1. Prepare data
  ...
  python ./scripts/update_river_params.py
  ```
  ...
  For convenience, we provide a dedicated script `scripts/make_runoff_map.py` to generate the mapping table using `DailyBinDataset`. You can modify it to use other dataset classes (like `NetCDFDataset` or `ERA5LandAccumDataset`) if needed.
  ...
  Several script templates are provided in the `./scripts` directory, such as `run_daily_bin.py` and `run_netcdf.py`. You are encouraged to modify or customize these scripts to fit your specific workflow and simulation requirements.
