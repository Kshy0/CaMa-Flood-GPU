...
def compute_flood_stage_kernel(
    ...
    river_area_ptr,              # *f32: River area
    max_flood_area_ptr,          # *f32: Max flood area
    ...
    flood_storage_updated = flood_storage + tl.where(river_storage_updated < 0.0, river_storage_updated, 0.0) + (flood_inflow - flood_outflow) * time_step
    ...
    river_area          = tl.load(river_area_ptr          + offs, mask=mask)
    max_flood_area      = tl.load(max_flood_area_ptr      + offs, mask=mask)
    ...
    for i in tl.static_range(num_flood_levels + 2):
        total_storage_i = tl.load(total_storage_table_ptr + offs * (level_max + 1) + i, mask=mask)
        level += tl.where(total_storage_i < total_storage, 1, 0)
    level = tl.maximum(level - 1, 0)

    prev_flood_depth   = tl.load(flood_depth_table_ptr   + offs * (level_max + 1) + level, mask=mask)
    prev_total_width   = tl.load(total_width_table_ptr   + offs * (level_max + 1) + level, mask=mask)
    prev_total_storage = tl.load(total_storage_table_ptr + offs * (level_max + 1) + level, mask=mask)
    flood_grad         = tl.load(flood_gradient_table_ptr+ offs * level_max + level, mask=mask)
