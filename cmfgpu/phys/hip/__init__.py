...
"""HIP backend for cmfgpu physics kernels.

JIT-compiles the hand-written HIP kernels and exposes dispatch functions
...
from hydroforge.runtime.hip_kernel import load_inline_hip_module as _load_inline_module
from hydroforge.runtime.cuda_kernel import precompile_extension_builders

_DIR = Path(__file__).resolve().parent
_BACKEND_KIND = "hip"
_MODULE_PREFIX = "cmfgpu_hip"
...
def _ext():
    ...
    src = (_DIR / "storage.hip").read_text()
    return _load_or_build(f"{_MODULE_PREFIX}_storage", cpp, src, ["-O3", "-ffast-math"],
                          ["launch_flood_stage"])
...
def _ext_outflow():
    ...
    Built **without** ``-ffast-math`` so that ``/`` and ``sqrt`` keep IEEE
    ...
    """
    src = (_DIR / "outflow.hip").read_text()
    cpp = (
    ...
    return _load_or_build(f"{_MODULE_PREFIX}_outflow", cpp, src, ["-O3", "-ffast-math"],
                          ["launch_outflow", "launch_inflow"])


@functools.lru_cache(maxsize=1)
def _ext_adaptive():
    src = (_DIR / "adaptive_time.hip").read_text()
    cpp = (
        "void launch_adaptive_time(at::Tensor,at::Tensor,at::Tensor,at::Tensor,"
        "double,double,double,long,long,long);\n"
    )
    return _load_or_build(f"{_MODULE_PREFIX}_adaptive", cpp, src, ["-O3", "-ffast-math"],
...
def _ext_bifurcation():
    ...
    src = (_DIR / "bifurcation.hip").read_text()
    cpp = (
    ...
    return _load_or_build(f"{_MODULE_PREFIX}_bifurcation", cpp, src, ["-O3", "-ffast-math"],
...
def _ext_reservoir():
    ...
    src = (_DIR / "reservoir.hip").read_text()
    cpp = (
    ...
    return _load_or_build(f"{_MODULE_PREFIX}_reservoir", cpp, src, ["-O3", "-ffast-math"],
                          ["launch_reservoir_outflow"])


@functools.lru_cache(maxsize=1)
def _ext_levee():
    """Levee stage + levee bifurcation outflow HIP kernels."""
    src = (_DIR / "levee.hip").read_text()
    cpp = (
    ...
    return _load_or_build(f"{_MODULE_PREFIX}_levee", cpp, src, ["-O3", "-ffast-math"],
...
def precompile_hip_extensions():
    """Precompile all cmfgpu HIP extensions via hydroforge's shared loader."""
    return precompile_extension_builders(
        __name__, _EXTENSION_BUILDERS, toolchain=_BACKEND_KIND
    )


precompile_cuda_extensions = precompile_hip_extensions
...
def compute_flood_stage(**kw):
    """Dispatch the fused storage-update + flood-stage HIP kernel.
...
# The log / batched variants are not yet ported to HIP; callers that need them
# should keep the Triton backend for those paths.
compute_flood_stage_log = None


def compute_outflow(**kw):
    """Dispatch the fused outflow + outgoing-storage HIP kernel.
...
def compute_inflow(**kw):
    """Dispatch the inflow-limiting + scatter HIP kernel.
...
def compute_adaptive_time_step(**kw):
    """Dispatch the CFL adaptive-time-step HIP kernel (per-thread atomicMax).
...
def compute_bifurcation_outflow(**kw):
    """Dispatch the bifurcation outflow HIP kernel."""
...
def compute_bifurcation_inflow(**kw):
    """Dispatch the bifurcation inflow (limiter + global scatter) HIP kernel."""
    ext = _ext_bifurcation()
    block = int(kw.get("BLOCK_SIZE", 256))
    num_paths = int(kw["num_bifurcation_paths"])
    num_levels = int(kw["num_bifurcation_levels"])
...
def compute_reservoir_outflow(**kw):
    """Dispatch the reservoir outflow HIP kernel.
...
def compute_levee_stage(**kw):
    """Dispatch the levee-aware flood-stage HIP kernel."""
    ext = _ext_levee()
    block = int(kw.get("BLOCK_SIZE", 256))
...
# Log / batched levee variants are not yet ported to HIP.  Matching storage,
# callers fall back to the non-log HIP stage when this is None.
compute_levee_stage_log = None


def compute_levee_bifurcation_outflow(**kw):
    """Dispatch the levee-aware bifurcation outflow HIP kernel."""
    ext = _ext_levee()
    block = int(kw.get("BLOCK_SIZE", 256))
