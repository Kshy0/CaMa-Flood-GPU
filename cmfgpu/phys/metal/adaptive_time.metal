...
kernel void compute_adaptive_time_step_batched(
    ...
    constant int&         num_catchments           [[buffer(7)]],
    constant int&         has_reservoir            [[buffer(8)]],
    constant int&         num_trials               [[buffer(9)]],
    constant int&         batched_downstream_distance [[buffer(10)]],
    uint idx [[thread_position_in_grid]],
    uint lid [[thread_position_in_threadgroup]],
    uint tpg [[threads_per_threadgroup]]
)
{
    ...
    if ((int)idx >= total) {
        ...
        int ds_off = batched_downstream_distance ? (trial_offset + catchment_idx) : catchment_idx;
