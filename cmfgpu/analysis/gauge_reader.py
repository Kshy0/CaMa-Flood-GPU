...
import numpy as np
try:
    import netCDF4 as nc  # optional, used only for meta nx/ny
...
class GaugeReader:
    ...
    def load_meta(
        ...
        parameter_nc: Optional[Union[str, Path]] = None,
        ...
        Notes:
          ...
          - catchment_id is computed as ix * ny + iy (C-order) using (nx, ny) from parameter_nc or provided nx/ny.
        """
        meta_path = Path(meta_txt)
        if not meta_path.exists():
            raise FileNotFoundError(f"Gauge meta file not found: {meta_path}")

        # Resolve map shape if not already set
        if self._map_shape is None:
            if parameter_nc is not None:
                self.load_map_shape_from_nc(parameter_nc)
            elif nx is not None and ny is not None:
                self.set_map_shape(nx, ny)
            else:
                raise ValueError("Map shape (nx, ny) required. Provide parameter_nc or nx/ny.")
