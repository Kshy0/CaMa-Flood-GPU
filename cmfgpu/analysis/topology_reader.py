...
class TopologyReader:
    ...
    def _load_data(self):
        ...
        with nc.Dataset(self.params_path, "r") as ds:
            ...
            if "nx" in ds.variables:
                self.nx = (
                    ds.variables["nx"][:].item()
                    if ds.variables["nx"].shape == ()
                    else ds.variables["nx"][0]
                )
            else:
                self.nx = None
            ...
            if "ny" in ds.variables:
                self.ny = (
                    ds.variables["ny"][:].item()
                    if ds.variables["ny"].shape == ()
                    else ds.variables["ny"][0]
                )
            else:
                self.ny = None
    ...
    def get_id_from_xy(self, x: int, y: int) -> int | None:
    ...
    def get_catchment_info(
        self, cid: int | None = None, xy: tuple[int, int] | None = None
    ) -> dict | None:
        ...
        info = {"index": int(idx), "upstream_ids": self._get_upstream(cid)}
    ...
    def visualize_basin(
        ...
        # Check for Lat/Lon
        use_lonlat = (
            "longitude" in self.catchment_data and "latitude" in self.catchment_data
        )
            ...
            for up in ups:
                if self._has_cid(up):
                    ...
                    if self.b_ids[u_idx] == basin_id:
                        area = (
                            self.catchment_data["upstream_area"][u_idx]
                            if "upstream_area" in self.catchment_data
                            else 0
                        )
                        if area > max_area:
    ...
    def print_topology(self, cid: int | None = None, xy: tuple[int, int] | None = None):
        """
