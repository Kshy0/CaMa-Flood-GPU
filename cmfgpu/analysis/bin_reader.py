...
class BinReader:
    ...
    def _load_dims(self):
        ...
        try:
            with mapdim_path.open("r") as f:
                for line in f:
                    ...
                    key = parts[0].lower()
                    if key in {"nx", "nxx"} and len(parts) >= 2 and self.nx is None:
                        self.nx = int(parts[1])
                    elif key in {"ny", "nyy"} and len(parts) >= 2 and self.ny is None:
                        self.ny = int(parts[1])
                    elif len(parts) == 1:
                        try:
        ...
        if self.nx is None or self.ny is None or self.nx <= 0 or self.ny <= 0:
            raise ValueError(f"{mapdim_path} does not define positive nx/ny dimensions")
    ...
    def _build_time_axis(self):
        ...
        for f in self.files:
            ...
            complete_frames = (
                seconds_in_year // self.dt if seconds_in_year % self.dt == 0 else None
            )
    ...
    def get_series(
        self,
        points: np.ndarray | Sequence[tuple[int, int]],
        fill_value: float = np.nan,
    ) -> np.ndarray:
        """
        ...
        for f_info in self.files:
            ...
            mmap = np.memmap(
                path, dtype="<f4", mode="r", shape=(n_frames, self.ny, self.nx)
            )

            # Extract points
            # mmap[:, ys, xs] uses fancy indexing on the last two dimensions
            chunk_data = mmap[:, ys, xs]
            missing = (chunk_data == np.float32(-9999.0)) | (
    ...
    def plot_single_time(
        ...
        t_str = (
            self.times[t_index].strftime("%Y-%m-%d %H:%M")
            if self.times
            else f"Index {t_index}"
        )
    ...
    def animate(
        ...
        t_end = (
            self._time_len if t_range is None else min(self._time_len, int(t_range[1]))
        )
        ...
        ani = animation.FuncAnimation(
            fig, update, frames=t_end - t_start, interval=1000 / fps, blit=False
        )
