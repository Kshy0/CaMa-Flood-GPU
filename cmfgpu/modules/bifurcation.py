...
from typing import ClassVar, Self, Optional, Tuple, Literal
...
class BifurcationModule(AbstractModule):
    ...
    # ------------------------------------------------------------------ #
    # Shared catchment IDs (from BaseModule)
    # ------------------------------------------------------------------ #
    bifurcation_path_id: torch.Tensor = BifurcationField(
        description="Unique ID for each bifurcation path",
        dtype="int",
    )
    ...
    bifurcation_elevation: torch.Tensor = BifurcationField(
        description="Channel-bed elevations by path and level (m)",
        shape=("num_bifurcation_paths", "num_bifurcation_levels"),
    )
    ...
    @computed_field(
        description="Number of vertical levels in each bifurcation path."
    )
    @cached_property
